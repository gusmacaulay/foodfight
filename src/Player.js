...
constructor(scene, camera, playerIndex = 0) {
    this.scene = scene;
    this.camera = camera;
    ...
    if (playerIndex === 1) {
        this.makeShadowBurger();
    }
}

makeShadowBurger() {
    this.mesh.traverse(child => {
        if (child.isMesh && !child.userData.isBite) {
            if (child.material.color) {
                child.material.color.multiplyScalar(0.3); // Make it dark
                child.material.color.b += 0.5; // Tint blue
            }
        }
    });
}
...
createBurger() {
    ...
    this.bunMat = new THREE.MeshStandardMaterial({ color: 0xffaa44 }); // Brownish bun
    const bottomBun = new THREE.Mesh(bunGeo, this.bunMat);
    bottomBun.position.y = 0.25;
    this.mesh.add(bottomBun);

    // Patty
    const pattyGeo = new THREE.CylinderGeometry(1.6, 1.6, 0.4, 16);
    this.pattyMat = new THREE.MeshStandardMaterial({ color: 0x441100 }); // Dark brown meat
    const patty = new THREE.Mesh(pattyGeo, this.pattyMat);
    patty.position.y = 0.7;
    this.mesh.add(patty);

    // Cheese
    const cheeseGeo = new THREE.BoxGeometry(3.1, 0.1, 3.1);
    this.cheeseMat = new THREE.MeshStandardMaterial({ color: 0xffdd00 }); // Yellow cheese
    const cheese = new THREE.Mesh(cheeseGeo, this.cheeseMat);
    cheese.position.y = 0.95;
    this.mesh.add(cheese);

    // Top Bun
    const topBunGeo = new THREE.SphereGeometry(1.5, 16, 16, 0, Math.PI * 2, 0, Math.PI / 2);
    const topBun = new THREE.Mesh(topBunGeo, this.bunMat);
...
updateBunColor(color) {
    if (this.bunMat) this.bunMat.color.set(color);
}

updatePattyColor(color) {
    if (this.pattyMat) this.pattyMat.color.set(color);
}

updateCheeseColor(color) {
    if (this.cheeseMat) this.cheeseMat.color.set(color);
}
